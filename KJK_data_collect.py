...
def load_documents(country_food):
    if country_food == '한식':
        ...
        with open(japanses_food, 'r', encoding='utf-8') as j:
            japanese_food_list = json.load(k)
...
def filter_text(text):
    ...
    text = text.lower()
    return text
query = "감자국국 레시피" #추후 계속 질문을 받는식으로 변경 가능
#----------------------------------------------------------------------
...
***Don't forget to indicate the numbers or weight of the food or ingredients***
...
2. 작은 볼에 튀김 가루를 넣고 찬물을 부어줍니다. 젓가락이나 거품기를 이용해 반죽을 3분 동안 가볍게 섞어줍니다. 반죽에 약간의 덩어리가 있어도 괜찮습니다.
